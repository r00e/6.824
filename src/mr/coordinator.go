...
var allTasksDone chan TaskType
...
// 没用上
func (c *Coordinator) RegisterWorkerHandler(args *RpcRegisterWorkerArgs, reply *RpcRegisterWorkerReply) error {
	uuid := args.Uuid
...
func (c *Coordinator) TaskHandler(args *TaskRpcArgs, reply *TaskRpcReply) error {
	...
	case pieceFileName := <-mapTasks:
		...
		c.RWLock.Unlock()
		go c.workerTimer(Map, pieceFileName, reply.MapTaskNum)
		return nil
...
func (c *Coordinator) workerTimer(taskType TaskType, taskInfo string, mapNum int) {
	...
	for {
		...
		case <-ticker.C:
			...
			case Reduce:
				...
				c.RWLock.Lock()
				c.allReduceTasks[idx] = unallocated
				c.RWLock.Unlock()
				reduceTasks <- idx
			}
			return
...
func (c *Coordinator) initServer(files []string, nReduce int) {
	...
	allTasksDone = make(chan TaskType, 1)
